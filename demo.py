...
def main(yolo):
    ...
    all_frames = []
    videos = ["./videos/init/Double1.mp4", "./videos/init/Single1.mp4"]
    for video in videos:
    ...
    for frame in all_frames:
        ...
        detections = [
            Detection(bbox, 1.0, feature) for bbox, feature in zip(boxs, features)
        ]  # length = n
    ...
    for i in images_by_id:
        ...
        feats[i] = reid._features(
            images_by_id[i]
        )  # reid._features(images_by_id[i][:min(len(images_by_id[i]),100)])
...
def get_FrameLabels(frame):
    text_scale = max(1, frame.shape[1] / 1600.0)
    text_thickness = 1 if text_scale > 1.1 else 1
    line_thickness = max(1, int(frame.shape[1] / 500.0))
    return text_scale, text_thickness, line_thickness


def cv2_addBox(
    track_id, frame, x1, y1, x2, y2, line_thickness, text_thickness, text_scale
):
...
def write_results(
    filename, data_type, w_frame_id, w_track_id, w_x1, w_y1, w_x2, w_y2, w_wid, w_hgt
):
    if data_type == "mot":
